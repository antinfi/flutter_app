...
class PrettyDioLogger extends Interceptor {
  ...
  //LoggerColor
  final infoColor = AnsiColor.fg(208);
  final paramColor = AnsiColor.fg(215);
  final successColor = AnsiColor.fg(77);
  final errorColor = AnsiColor.fg(196);
  ...
  Future onRequest(RequestOptions options, RequestInterceptorHandler handler) async {
    ...
    if (requestHeader) {
      ...
      final requestHeaders = {}..addAll(options.headers);
  ...
  Future onError(DioError err, ErrorInterceptorHandler handler) async {
    if (error) {
      ...
      if (err.type == DioErrorType.badResponse ) {
        _printBoxed(header: 'DioError ║ Status: ${err.response!.statusCode} ${err.response!.statusMessage}', text: uri.toString(), color: errorColor);
  ...
  Future onResponse(Response response, ResponseInterceptorHandler handler) async {
    final iColor = response.statusCode == 200 ? successColor : AnsiColor.fg(209);
  ...
  void _printResponseHeader(Response response, AnsiColor vColor) {
    ...
    _printBoxed(header: 'Response ║ $method ║ Status: ${response.statusCode} ${response.statusMessage}', text: uri.toString(), color: vColor);
  }
  ...
  void _printBlock(String msg, {AnsiColor? color}) {
    ...
    for (var i = 0; i < lines; ++i) {
      logPrint(color((i >= 0 ? '║ ' : '') + msg.substring(i * maxWidth, math.min<int>(i * maxWidth + maxWidth, msg.length))));
    }
  }

  String _indent([int tabCount = initialTab]) => tabStep * tabCount;

  void _printPrettyMap(Map data, {int tabs = initialTab, bool isListItem = false, bool isLast = false, AnsiColor? vColor}) {
    ...
    data.keys.toList().asMap().forEach((index, key) {
      ...
      var value = data[key];
      if (value is String) value = '"${value.toString().replaceAll(RegExp(r'([\r\n])+'), " ")}"';
      if (value is Map) {
        ...
        if (msg.length + indent.length > linWidth) {
          ...
          for (var i = 0; i < lines; ++i) {
            logPrint(vColor!('║${_indent(tabs)} ${msg.substring(i * linWidth, math.min<int>(i * linWidth + linWidth, msg.length))}'));
          }
        } else {
  ...
  bool _canFlattenMap(Map map) => map.values.where((val) => val is Map || val is List).isEmpty && map.toString().length < maxWidth;
  ...
  void _printMapAsTable(Map? map, {String? header, AnsiColor? color}) {
    ...
    logPrint(color('╔ $header '));
    map.forEach((key, value) => _printKV(key, value, color: color));
    _printLine(pre: '╚', color: color);
  }
}
